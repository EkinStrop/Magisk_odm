...
│   ├── odm                 <--- A symlink to $MODID/system/odm
...
If your device exposes a standalone `/odm` mount point, place ODM files under `system/odm`. On devices where ODM is merged into `/vendor`, keep using `system/vendor/odm`.
