...
private fun Project.downloadFile(url: String, checksum: String): File {
    val file = rootFile(".gradle/downloads/$checksum")
    if (file.exists()) {
...
private fun Project.verifyNativeBinaries(abiList: Set<String>) {
    val expectedVersionTag = "${Config.version}:MAGISK:"
    val missingOrStale = abiList.firstOrNull { abi ->
        val abiDir = rootFile("native/out/$abi")
        val missingBinary = CORE_NATIVE_BINARIES.any { binary -> !abiDir.resolve(binary).isFile }
        if (missingBinary) {
            return@firstOrNull true
        }
        val magiskBin = abiDir.resolve("magisk")
        val magiskText = magiskBin.readText(StandardCharsets.ISO_8859_1)
        !magiskText.contains(expectedVersionTag) || !magiskText.contains(Config.versionCode.toString())
    }
    if (missingOrStale != null) {
        throw StopExecutionException(
            "Native binaries are missing or stale for $missingOrStale. Rebuild them first with ./build.py native"
        )
    }
}
...
private val CORE_NATIVE_BINARIES =
    listOf("magiskboot", "magiskinit", "magiskpolicy", "magisk", "libinit-ld.so")
...
fun Project.setupMainApk() {
    ...
    androidApp {
        ...
        defaultConfig {
            applicationId = "com.topjohnwu.odmisk"
