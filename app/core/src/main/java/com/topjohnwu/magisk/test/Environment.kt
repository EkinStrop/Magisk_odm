...
class Environment : BaseTest {
    ...
    private fun setupMountTest(root: ExtendedFile) {
        ...
        if (Shell.cmd("[ -d /odm ]").exec().isSuccess) {
            val odm = path.getChildFile("system").getChildFile("odm").getChildFile("etc")
            assertTrue(error, odm.mkdirs())
            assertTrue(error, odm.getChildFile("magisk-odm-test").createNewFile())
        }
